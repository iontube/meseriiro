...
function hd(x,y,t,s){return `<rect x="${x}" y="${y-2}" width="6" height="30" rx="3" fill="url(#gW)"/>
 ...
 <text x="${x+18}" y="${y+42}" font-size="15.5" font-weight="500" fill="${C.sub}">${s}</text>`;}
...
function card(W,H,inner){return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="100%" style="height:auto;display:block;max-width:${W}px;margin:1.5rem auto" font-family="${FONT}" role="img">
 ...
 ${inner}</svg>`;}
function vgrid(x0,x1,yTop,yBot,max,step,unit=''){let g='';for(let v=0;v<=max;v+=step){const x=x0+(v/max)*(x1-x0);g+=`<line x1="${x}" y1="${yTop}" x2="${x}" y2="${yBot}" stroke="${C.grid}" stroke-width="1" stroke-dasharray="2 5"/><text x="${x}" y="${yBot+18}" text-anchor="middle" font-size="12" font-weight="600" fill="${C.faint}">${v/1000}${unit||'k'}</text>`;}return g;}
...
K.range=()=>{const W=760,H=380,px=48,x0=px+140,x1=W-px-16,mx=13000,pyTop=110,rowH=70;
 ...
 R.forEach((r,i)=>{const y=pyTop+i*rowH+6;g+=`<text x="${px}" y="${y}" font-size="17" font-weight="800" fill="${C.ink}">${r[0]}</text><text x="${px}" y="${y+18}" font-size="12.5" font-weight="600" fill="${C.sub}">${r[1]}</text>`;
  ...
  g+=`<text x="${sx(r[2])}" y="${y-18}" text-anchor="middle" font-size="13.5" font-weight="800" fill="${C.ink}">${fmt(r[2])}</text><text x="${sx(r[4])}" y="${y-18}" text-anchor="middle" font-size="13.5" font-weight="800" fill="${C.a1}">${fmt(r[4])}</text>`;});
...
K.ranked=()=>{const W=760,H=400,px=48,x0=px+48,x1=W-px-64,rowH=52,pyTop=112;
 ...
 R.forEach((r,i)=>{const y=pyTop+i*rowH,gr=i===0?'url(#gN)':'url(#gW)';
  g+=`<circle cx="${px+15}" cy="${y+16}" r="15" fill="${i===0?C.navy:'#FBECE6'}"/><text x="${px+15}" y="${y+21}" text-anchor="middle" font-size="15" font-weight="800" fill="${i===0?'#fff':C.a1}">${i+1}</text>`;
  g+=`<text x="${x0}" y="${y+1}" font-size="15" font-weight="700" fill="${C.ink}">${r[0]}</text>`;
  g+=`<rect x="${x0}" y="${y+9}" width="${x1-x0}" height="19" rx="8" fill="${C.track}"/><rect x="${x0}" y="${y+9}" width="${bw(r[1])}" height="19" rx="8" fill="${gr}"/>`;
  g+=`<text x="${x0+bw(r[1])+10}" y="${y+23}" font-size="14" font-weight="800" fill="${i===0?C.navy:C.a1}">${i===0?'100':r[1]}</text>`;});
...
K.segmented=()=>{const W=760,H=300,px=48,x0=px,x1=W-px,bY=140,bH=44;
 ...
 S.forEach((s,i)=>{const w=(s[1]/tot)*(x1-x0);g+=`<rect x="${cx}" y="${bY}" width="${w}" height="${bH}" fill="${col[i]}" ${i===0?'rx="10"':''}/>`;if(i===S.length-1)g+=`<rect x="${cx+w-14}" y="${bY}" width="14" height="${bH}" rx="10" fill="${col[i]}"/>`;
  g+=`<text x="${cx+w/2}" y="${bY+bH/2+5}" text-anchor="middle" font-size="13.5" font-weight="800" fill="#fff">$${fmt(s[1])}</text><text x="${cx+w/2}" y="${bY-12}" text-anchor="middle" font-size="12.5" font-weight="700" fill="${C.slate}">${s[0]}</text>`;cx+=w;});
 g+=`<text x="${x1}" y="${bY+bH+42}" text-anchor="end" font-size="14" font-weight="600" fill="${C.sub}">Total</text><text x="${x1}" y="${bY+bH+70}" text-anchor="end" font-size="28" font-weight="800" fill="${C.ink}">≈ $${fmt(tot)}</text>`;
...
K.donut=()=>{const W=760,H=352,cx=210,cy=212,R=100,r=60,brut=12000;const cas=brut*.25,cass=brut*.1,imp=(brut-cas-cass)*.1,net=brut-cas-cass-imp;
 ...
 g+=`<text x="${cx}" y="${cy-4}" text-anchor="middle" font-size="14" font-weight="600" fill="${C.sub}">NET</text><text x="${cx}" y="${cy+24}" text-anchor="middle" font-size="28" font-weight="800" fill="${C.ink}">${fmt(net)}</text>`;
 let ly=108;P4.forEach(p=>{g+=`<rect x="410" y="${ly-13}" width="15" height="15" rx="4" fill="${p[2]}"/><text x="434" y="${ly}" font-size="14.5" font-weight="700" fill="${C.ink}">${p[0]}</text><text x="${W-48}" y="${ly}" text-anchor="end" font-size="14.5" font-weight="800" fill="${p[2]}">${fmt(p[1])}</text>`;ly+=40;});
...
K.staircase=()=>{const W=760,H=380,px=48,baseY=300;const S=[['Junior',5900],['Analist',8000],['Senior',11200],['Manager',15000,1],['CFO',24000,1]];
 ...
 S.forEach((s,i)=>{const bx=x0+i*sw,h=sh(s[1]),by=baseY-h;g+=`<rect x="${bx+12}" y="${by}" width="${sw-24}" height="${h}" rx="9" fill="url(#gWv)" opacity="${s[2]?0.55:1}"/>`;
  g+=`<text x="${bx+sw/2}" y="${by-12}" text-anchor="middle" font-size="15" font-weight="800" fill="${s[2]?C.a2:C.a1}">${fmt(s[1])}</text>`;
  g+=`<text x="${bx+sw/2}" y="${baseY+22}" text-anchor="middle" font-size="12.5" font-weight="700" fill="${C.ink}">${s[0]}</text>`;poly+=`${bx+sw/2},${by-2} `;});
...
K.grouped=()=>{const W=760,H=380,px=48,x0=px+140,x1=W-px-16,mx=16000,pyTop=108,rowH=64;
 ...
 R.forEach((r,i)=>{const y=pyTop+i*rowH;g+=`<text x="${px}" y="${y+18}" font-size="15" font-weight="700" fill="${C.ink}">${r[0]}</text>`;
  g+=`<rect x="${x0}" y="${y}" width="${sx(r[1])-x0}" height="15" rx="5" fill="url(#gW)"/><text x="${sx(r[1])+8}" y="${y+13}" font-size="12.5" font-weight="800" fill="${C.a1}">${fmt(r[1])}</text>`;
  g+=`<rect x="${x0}" y="${y+22}" width="${sx(r[2])-x0}" height="15" rx="5" fill="url(#gN)"/><text x="${sx(r[2])+8}" y="${y+35}" font-size="12.5" font-weight="800" fill="${C.navy}">${fmt(r[2])}</text>`;});
 g+=`<rect x="${px}" y="${H-40}" width="13" height="13" rx="3" fill="url(#gW)"/><text x="${px+19}" y="${H-29}" font-size="13" font-weight="600" fill="${C.sub}">România</text><rect x="${px+140}" y="${H-40}" width="13" height="13" rx="3" fill="url(#gN)"/><text x="${px+159}" y="${H-29}" font-size="13" font-weight="600" fill="${C.sub}">Străinătate</text>`;
 return card(W,H,g);};
...
K.hbars=()=>{const W=760,H=380,px=48,x0=px+150,x1=W-px-56,mx=8000,pyTop=104,rowH=46;
 ...
 R.forEach((r,i)=>{const y=pyTop+i*rowH;g+=`<text x="${px}" y="${y+18}" font-size="14.5" font-weight="700" fill="${C.ink}">${r[0]}</text>`;
  g+=`<rect x="${x0}" y="${y+3}" width="${x1-x0}" height="20" rx="7" fill="${C.track}"/><rect x="${x0}" y="${y+3}" width="${bw(r[1])}" height="20" rx="7" fill="url(#gW)"/><text x="${x0+bw(r[1])+10}" y="${y+18}" font-size="14" font-weight="800" fill="${C.a1}">${fmt(r[1])}</text>`;});
...
K.waterfall=()=>{const W=760,H=380,px=48,baseY=300,x0=px+6,x1=W-px-6;
 ...
 steps.forEach((s,i)=>{const bx=x0+i*sw+12,w=sw-24;let top,h,col;
  ...
  g+=`<text x="${bx+w/2}" y="${top-8}" text-anchor="middle" font-size="13" font-weight="800" fill="${s[2]==='neg'?C.slate:C.ink}">${s[2]==='neg'?'':''}${fmt(Math.abs(s[1]))}</text>`;
  g+=`<text x="${bx+w/2}" y="${baseY+22}" text-anchor="middle" font-size="12.5" font-weight="700" fill="${C.ink}">${s[0]}</text>`;});
...
K.line=()=>{const W=760,H=360,px=48,x0=px+40,x1=W-px-16,yT=100,yB=280;
 ...
 for(let v=4000;v<=7000;v+=1000){const y=sy(v);g+=`<line x1="${x0}" y1="${y}" x2="${x1}" y2="${y}" stroke="${C.grid}" stroke-width="1" stroke-dasharray="2 5"/><text x="${x0-10}" y="${y+4}" text-anchor="end" font-size="11.5" font-weight="600" fill="${C.faint}">${v/1000}k</text>`;}
 ...
 D.forEach((d,i)=>{g+=`<circle cx="${sx(i)}" cy="${sy(d[1])}" r="5" fill="#fff" stroke="${C.a1}" stroke-width="3"/><text x="${sx(i)}" y="${yB+22}" text-anchor="middle" font-size="12.5" font-weight="700" fill="${C.sub}">${d[0]}</text>`;});
 g+=`<text x="${sx(D.length-1)}" y="${sy(D[D.length-1][1])-14}" text-anchor="middle" font-size="13.5" font-weight="800" fill="${C.a1}">${fmt(D[D.length-1][1])}</text>`;
...
K.lollipop=()=>{const W=760,H=380,px=48,x0=px+150,x1=W-px-56,mx=13000,pyTop=104,rowH=48;
 ...
 R.forEach((r,i)=>{const y=pyTop+i*rowH+8;g+=`<text x="${px}" y="${y+4}" font-size="14" font-weight="700" fill="${C.ink}">${r[0]}</text>`;
  ...
  g+=`<circle cx="${sx(r[1])}" cy="${y}" r="11" fill="${C.a1}"/><text x="${sx(r[1])+20}" y="${y+5}" font-size="14" font-weight="800" fill="${C.ink}">${fmt(r[1])}</text>`;});
 return card(W,H,g+cap(px,H-24,'Sursă: date piață'));};
...
K.bullet=()=>{const W=760,H=360,px=48,x0=px+150,x1=W-px-40,mx=14000,pyTop=112,rowH=64;
 ...
 R.forEach((r,i)=>{const y=pyTop+i*rowH;g+=`<text x="${px}" y="${y+22}" font-size="14.5" font-weight="700" fill="${C.ink}">${r[0]}</text>`;
  ...
  g+=`<text x="${sx(r[1])+8}" y="${y+26}" font-size="13.5" font-weight="800" fill="${C.a1}">${fmt(r[1])}</text>`;});
 g+=`<line x1="${px}" y1="${H-38}" x2="${px+18}" y2="${H-38}" stroke="${C.navy}" stroke-width="3"/><text x="${px+26}" y="${H-33}" font-size="13" font-weight="600" fill="${C.sub}">reper / țintă</text>`;
 return card(W,H,g);};
...
K.gauge=()=>{const W=760,H=320,cx=250,cy=250,R=150,pct=0.585;
 ...
 g+=`<text x="${cx}" y="${cy-18}" text-anchor="middle" font-size="46" font-weight="800" fill="${C.ink}">58,5%</text><text x="${cx}" y="${cy+12}" text-anchor="middle" font-size="14.5" font-weight="600" fill="${C.sub}">net din brut (peste prag)</text>`;
 g+=`<text x="${cx-R}" y="${cy+26}" text-anchor="middle" font-size="12" font-weight="600" fill="${C.faint}">0%</text><text x="${cx+R}" y="${cy+26}" text-anchor="middle" font-size="12" font-weight="600" fill="${C.faint}">100%</text>`;
...
K.slope=()=>{const W=760,H=380,px=48,xL=px+120,xR=W-px-120,yT=126,yB=302,mx=20000,mn=4000;
 ...
 g+=`<text x="${xL}" y="${yT-16}" text-anchor="middle" font-size="14" font-weight="700" fill="${C.sub}">Fără CFA</text><text x="${xR}" y="${yT-16}" text-anchor="middle" font-size="14" font-weight="700" fill="${C.sub}">Cu CFA</text>`;
 [[8000,13000,C.slate,'Level 1'],[11000,17000,C.a1,'Charter']].forEach((s,i)=>{g+=`<line x1="${xL}" y1="${sy(s[0])}" x2="${xR}" y2="${sy(s[1])}" stroke="${s[2]}" stroke-width="3.5"/>`;
  ...
  g+=`<text x="${xL-12}" y="${sy(s[0])+4}" text-anchor="end" font-size="13" font-weight="800" fill="${s[2]}">${fmt(s[0])}</text><text x="${xR+12}" y="${sy(s[1])+4}" font-size="13" font-weight="800" fill="${s[2]}">${fmt(s[1])}</text>`;});
...
K.stats=()=>{const W=760,H=260,px=40;const S=[['8.500 lei','net mediu/lună',C.a1],['+59%','cu CFA (global)',C.navy],['~971','joburi active RO',C.teal],['3,5','poziție Google',C.gold]];
 ...
 S.forEach((s,i)=>{const x=px+i*(cw+10),y=118;g+=`<rect x="${x}" y="${y}" width="${cw}" height="100" rx="14" fill="${C.canvas}" stroke="${C.border}"/><rect x="${x}" y="${y}" width="5" height="100" rx="2.5" fill="${s[2]}"/>`;
  g+=`<text x="${x+cw/2+3}" y="${y+48}" text-anchor="middle" font-size="26" font-weight="800" fill="${C.ink}">${s[0]}</text><text x="${x+cw/2+3}" y="${y+74}" text-anchor="middle" font-size="12.5" font-weight="600" fill="${C.sub}">${s[1]}</text>`;});
 return card(W,H,g);};
...
K.pictogram=()=>{const W=760,H=300,px=48;let g=hd(px,38,'Din 10 analiști','câți depășesc 9.000 lei net');
 ...
 g+=`<text x="${W-px}" y="150" text-anchor="end" font-size="40" font-weight="800" fill="${C.a1}">4 din 10</text><text x="${W-px}" y="178" text-anchor="end" font-size="14" font-weight="600" fill="${C.sub}">peste 9.000 lei net</text>`;
...
K.diverging=()=>{const W=760,H=380,px=48,midX=W/2,mx=4000,pyTop=110,rowH=48,half=(W-px*2)/2-30;
 ...
 R.forEach((r,i)=>{const y=pyTop+i*rowH,pos=r[1]>=0;g+=`<text x="${midX+(pos?-6:6)}" y="${y+15}" text-anchor="${pos?'end':'start'}" font-size="13.5" font-weight="700" fill="${C.ink}">${r[0]}</text>`;
  if(pos)g+=`<rect x="${midX}" y="${y+4}" width="${bw(r[1])}" height="18" rx="5" fill="url(#gW)"/><text x="${midX+bw(r[1])+8}" y="${y+18}" font-size="13" font-weight="800" fill="${C.a1}">+${fmt(r[1])}</text>`;
  else g+=`<rect x="${midX-bw(r[1])}" y="${y+4}" width="${bw(r[1])}" height="18" rx="5" fill="url(#gN)"/><text x="${midX-bw(r[1])-8}" y="${y+18}" text-anchor="end" font-size="13" font-weight="800" fill="${C.navy}">${fmt(r[1])}</text>`;});
 return card(W,H,g+cap(px,H-24,'Sursă: date piață'));};
...
K.funnel=()=>{const W=760,H=380,px=48,cx=W/2,yTop=100,rowH=52;const R=[['Junior',100],['Analist',68],['Senior',40],['Manager',18],['Director',6]];const maxW=W-px*2-120;
 ...
 R.forEach((r,i)=>{const w=(r[1]/100)*maxW,y=yTop+i*rowH,gr=i<2?'url(#gW)':'url(#gN)';g+=`<rect x="${cx-w/2}" y="${y}" width="${w}" height="${rowH-12}" rx="8" fill="${gr}"/>`;
  g+=`<text x="${cx}" y="${y+26}" text-anchor="middle" font-size="14.5" font-weight="800" fill="#fff">${r[0]}</text>`;g+=`<text x="${cx+w/2+12}" y="${y+26}" font-size="13" font-weight="700" fill="${C.sub}">${r[1]}%</text>`;});
...
K.rings=()=>{const W=760,H=320,px=48;const D=[['Excel',0.9,C.a1],['SQL',0.7,C.navy],['Python',0.45,C.teal],['Engleză',0.85,C.gold]];
 ...
 D.forEach((d,i)=>{const cx=px+i*cw+cw/2,cy=180,R=46;const circ=2*P*R,off=circ*(1-d[1]);
  ...
  g+=`<text x="${cx}" y="${cy+6}" text-anchor="middle" font-size="19" font-weight="800" fill="${C.ink}">${Math.round(d[1]*100)}%</text><text x="${cx}" y="${cy+72}" text-anchor="middle" font-size="14" font-weight="700" fill="${C.slate}">${d[0]}</text>`;});
...
K.rangecol=()=>{const W=760,H=380,px=48,baseY=300,x0=px+6,x1=W-px-6;const D=[['Junior',5300,6500],['Mediu',7000,9000],['Senior',10000,12500],['Lead',13000,18000]];
 ...
 for(let v=0;v<=20000;v+=5000){const y=sy(v);g+=`<line x1="${x0}" y1="${y}" x2="${x1}" y2="${y}" stroke="${C.grid}" stroke-width="1" stroke-dasharray="2 5"/><text x="${x0-6}" y="${y+4}" text-anchor="end" font-size="11" font-weight="600" fill="${C.faint}">${v/1000}k</text>`;}
 D.forEach((d,i)=>{const bx=x0+i*sw+sw/2,top=sy(d[2]),bot=sy(d[1]);g+=`<rect x="${bx-22}" y="${top}" width="44" height="${bot-top}" rx="8" fill="url(#gWv)"/>`;
  g+=`<text x="${bx}" y="${top-8}" text-anchor="middle" font-size="12.5" font-weight="800" fill="${C.a1}">${fmt(d[2])}</text><text x="${bx}" y="${bot+18}" text-anchor="middle" font-size="12" font-weight="700" fill="${C.sub}">${fmt(d[1])}</text><text x="${bx}" y="${baseY+22}" text-anchor="middle" font-size="12.5" font-weight="700" fill="${C.ink}">${d[0]}</text>`;});
 g+=`<line x1="${x0}" y1="${baseY}" x2="${x1}" y2="${baseY}" stroke="${C.grid}" stroke-width="2"/>`;
 return card(W,H,g);};
...
K.stack100=()=>{const W=760,H=300,px=48,x0=px,x1=W-px,bY=150,bH=46;const S=[['Salariu bază',62,C.navy],['Bonus/prime',20,C.a1],['Tichete',10,C.teal],['Alte beneficii',8,C.gold]];
 ...
 S.forEach((s,i)=>{const w=(s[1]/100)*(x1-x0);g+=`<rect x="${cx}" y="${bY}" width="${w}" height="${bH}" fill="${s[2]}" ${i===0?'rx="10"':''}/>`;if(i===S.length-1)g+=`<rect x="${cx+w-14}" y="${bY}" width="14" height="${bH}" rx="10" fill="${s[2]}"/>`;
  if(w>50)g+=`<text x="${cx+w/2}" y="${bY+bH/2+5}" text-anchor="middle" font-size="13" font-weight="800" fill="#fff">${s[1]}%</text>`;cx+=w;});
 let ly=bY+bH+34;let lx=px;S.forEach(s=>{g+=`<rect x="${lx}" y="${ly-12}" width="13" height="13" rx="3" fill="${s[2]}"/><text x="${lx+19}" y="${ly}" font-size="13" font-weight="600" fill="${C.sub}">${s[0]} ${s[1]}%</text>`;lx+=165;});
