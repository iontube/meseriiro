// Charturi "pro" pentru articolul Analist Financiar — 7 tipuri diverse, design system coerent.
// v2 (2026-07-14): viewBox 880->760 + PODEA DE FONT pt lizibilitate pe mobil (niciun text sub ~15u -> ~7px real).
...
function header(x,y,W,title,sub){return `
  ...
  <text x="${x+18}" y="${y+42}" font-size="17" font-weight="500" fill="${C.sub}">${sub}</text>`;}
...
function c_experienta(){
  const W=760,H=440,px=52,pyTop=120,rowH=74;
  ...
  const x0=px+140,x1=W-px-20,maxV=14000,sx=v=>x0+(v/maxV)*(x1-x0);
  let g=header(px,40,W,'Salariu net pe experiență','Analist financiar în România, lei net pe lună (2026)');
  for(let v=0;v<=14000;v+=2000){const x=sx(v);g+=`<line x1="${x}" y1="${pyTop-14}" x2="${x}" y2="${pyTop+rows.length*rowH-24}" stroke="${C.grid}" stroke-width="1" stroke-dasharray="2 5"/><text x="${x}" y="${pyTop+rows.length*rowH-4}" text-anchor="middle" font-size="15" font-weight="600" fill="${C.faint}">${v/1000}k</text>`;}
  rows.forEach((r,i)=>{const y=pyTop+i*rowH+8;
    g+=`<text x="${px}" y="${y+2}" font-size="19" font-weight="800" fill="${C.ink}">${r.label}</text><text x="${px}" y="${y+22}" font-size="15" font-weight="600" fill="${C.sub}">${r.sub}</text>`;
    ...
    g+=`<text x="${sx(r.min)}" y="${y-16}" text-anchor="middle" font-size="16.5" font-weight="800" fill="${C.ink}">${fmt(r.min)}</text><text x="${sx(r.max)}" y="${y-16}" text-anchor="middle" font-size="16.5" font-weight="800" fill="${C.a1}">${fmt(r.max)}</text>`;});
  const ly=pyTop+rows.length*rowH+34;
  g+=`<rect x="${px}" y="${ly-11}" width="13" height="13" rx="6.5" fill="url(#dumb)"/><text x="${px+20}" y="${ly}" font-size="15.5" font-weight="600" fill="${C.sub}">interval de piață</text>`;
  g+=`<rect x="${px+188}" y="${ly-10}" width="12" height="12" rx="2" transform="rotate(45 ${px+194} ${ly-4})" fill="#fff" stroke="${C.navy}" stroke-width="2"/><text x="${px+212}" y="${ly}" font-size="15.5" font-weight="600" fill="${C.sub}">valoare tipică</text>`;
...
// 2) RANKED — pe sector
function c_sector(){
  const W=760,H=452,px=52,pyTop=124,rowH=54;
  ...
  rows.forEach((r,i)=>{const y=pyTop+i*rowH,grad=i===0?'url(#barNavy)':'url(#barWarm)';
    g+=`<circle cx="${px+16}" cy="${y+17}" r="16" fill="${i===0?C.navy:'#FBECE6'}"/><text x="${px+16}" y="${y+22}" text-anchor="middle" font-size="17" font-weight="800" fill="${i===0?'#fff':C.a1}">${i+1}</text>`;
    g+=`<text x="${x0}" y="${y+2}" font-size="17" font-weight="700" fill="${C.ink}">${r.label}</text>`;
    g+=`<rect x="${x0}" y="${y+10}" width="${x1-x0}" height="20" rx="8" fill="${C.track}"/><rect x="${x0}" y="${y+10}" width="${bw(r.val)}" height="20" rx="8" fill="${grad}"/>`;
    g+=`<text x="${x0+bw(r.val)+12}" y="${y+26}" font-size="16.5" font-weight="800" fill="${i===0?C.navy:C.a1}">${i===0?'reper 100':r.val}</text>`;
    if(r.note)g+=`<text x="${x0}" y="${y+47}" font-size="15" font-weight="500" fill="${C.faint}">${r.note}</text>`;});
  g+=cap(px,H-24,'Ordonat după INS + practica pieței · nivel relativ, nu cifre absolute');
  return card(W,H,g);
}

// 3) SEGMENTED BAR — costul CFA (3 nivele + materiale) spre total
function c_cfa(){
  const W=760,H=340,px=52;
  const segs=[{label:'Nivel 1',v:1140},{label:'Nivel 2',v:1140},{label:'Nivel 3',v:1240},{label:'Materiale',v:600}];
  ...
  segs.forEach((s,i)=>{const w=sw(s.v);
    ...
    g+=`<text x="${cx+w/2}" y="${barY+barH/2+5}" text-anchor="middle" font-size="16" font-weight="800" fill="#fff">$${fmt(s.v)}</text>`;
    g+=`<text x="${cx+w/2}" y="${barY-14}" text-anchor="middle" font-size="15.5" font-weight="700" fill="${C.slate}">${s.label}</text>`;
    cx+=w;});
  // total callout
  g+=`<text x="${x1}" y="${barY+barH+40}" text-anchor="end" font-size="16" font-weight="600" fill="${C.sub}">Total drum complet</text>`;
  g+=`<text x="${x1}" y="${barY+barH+70}" text-anchor="end" font-size="30" font-weight="800" fill="${C.ink}">≈ $${fmt(total)}</text>`;
  g+=`<text x="${x0}" y="${barY+barH+44}" font-size="16" font-weight="600" fill="${C.slate}">Durată medie</text>`;
  g+=`<text x="${x0}" y="${barY+barH+72}" font-size="22" font-weight="800" fill="${C.a1}">~4 ani</text>`;
  g+=cap(px,H-24,'Sursă: CFA Institute, taxe examen 2026 · charterul cere 4.000 h experiență');
  return card(W,H,g);
}

// 4) DONUT — brut → net (12.000 lei brut, 2026)
function c_brutnet(){
  const W=760,H=400,cx=205,cy=228,R=100,r=62;
  ...
  parts.forEach(p=>{const frac=p.v/brut,a1=a0+frac*2*Math.PI;
    ...
    a0=a1;});
  g+=`<text x="${cx}" y="${cy-6}" text-anchor="middle" font-size="16.5" font-weight="600" fill="${C.sub}">NET</text>`;
  g+=`<text x="${cx}" y="${cy+24}" text-anchor="middle" font-size="30" font-weight="800" fill="${C.ink}">${fmt(net)}</text>`;
  // legend
  let ly=120;parts.forEach(p=>{g+=`<rect x="360" y="${ly-14}" width="16" height="16" rx="4" fill="${p.cs}"/><text x="386" y="${ly}" font-size="16.5" font-weight="700" fill="${C.ink}">${p.label}</text><text x="${W-38}" y="${ly}" text-anchor="end" font-size="16.5" font-weight="800" fill="${p.cs}">${fmt(p.v)} lei</text>`;ly+=44;});
  g+=cap(52,H-24,'Formula 2026: CAS 25% + CASS 10% + impozit 10% · fără scutiri sectoriale');
  return card(W,H,g);
}

// 5) STAIRCASE — cariera
function c_cariera(){
  const W=760,H=420,px=52,baseY=330;
  ...
  steps.forEach((s,i)=>{const bx=x0+i*sw,h=sh(s.v),by=baseY-h;
    ...
    g+=`<text x="${bx+sw/2}" y="${by-26}" text-anchor="middle" font-size="18" font-weight="800" fill="${s.est?C.a2:C.a1}">${fmt(s.v)}</text>`;
    if(s.est)g+=`<text x="${bx+sw/2}" y="${by-10}" text-anchor="middle" font-size="14.5" font-weight="600" fill="${C.faint}">estimativ</text>`;
    // role label (wrap)
    const words=s.label.replace('&amp;','&').split(' ');
    let line1=words.slice(0,2).join(' '),line2=words.slice(2).join(' ');
    g+=`<text x="${bx+sw/2}" y="${baseY+24}" text-anchor="middle" font-size="15" font-weight="700" fill="${C.ink}">${line1.replace('&','&amp;')}</text>`;
    if(line2)g+=`<text x="${bx+sw/2}" y="${baseY+42}" text-anchor="middle" font-size="15" font-weight="700" fill="${C.ink}">${line2.replace('&','&amp;')}</text>`;});
  g+=`<line x1="${x0}" y1="${baseY}" x2="${x1}" y2="${baseY}" stroke="${C.grid}" stroke-width="2"/>`;
  g+=cap(px,H-24,'Surse: piață RO + INS · treptele de management sunt estimative');
  return card(W,H,g);
}

// 6) STAT CARDS — cifrele cheie
function c_stats(){
  const W=760,H=250,px=44;
  ...
  S.forEach((s,i)=>{const x=px+i*(cw+10),y=110;
    ...
    g+=`<text x="${x+cw/2+3}" y="${y+48}" text-anchor="middle" font-size="26" font-weight="800" fill="${C.ink}">${s[0]}</text><text x="${x+cw/2+3}" y="${y+76}" text-anchor="middle" font-size="15" font-weight="600" fill="${C.sub}">${s[1]}</text>`;});
  g+=cap(px,H-22,'Surse: INS, Paylab, BestJobs, CFA Institute · ultima cifră e globală (nu RO)');
  return card(W,H,g);
}
// 7) LOLLIPOP — comparație roluri înrudite
function c_roluri(){
  const W=760,H=380,px=52,x0=px+180,x1=W-px-64,mx=13000,pyTop=110,rowH=48;
  ...
  R.forEach((r,i)=>{const y=pyTop+i*rowH+8,hl=r[0]==='Analist financiar';
    g+=`<text x="${px}" y="${y+5}" font-size="16" font-weight="${hl?'800':'700'}" fill="${hl?C.a1:C.ink}">${r[0]}</text>`;
    ...
    g+=`<text x="${sx(r[1])+22}" y="${y+5}" font-size="16.5" font-weight="800" fill="${C.ink}">${fmt(r[1])}</text>`;});
...
// preview PNGs — render la latime MOBIL (390px) ca sa verific lizibilitatea reala
import { execSync } from 'child_process';
for(const [k,svg] of Object.entries(out)){writeFileSync(`/tmp/ch-${k}.svg`,svg);execSync(`rsvg-convert -w 390 /tmp/ch-${k}.svg -o /tmp/ch-${k}.png`);}
console.log('✓ 7 charturi generate (mobil-preview 390px):', Object.keys(out).join(', '));
